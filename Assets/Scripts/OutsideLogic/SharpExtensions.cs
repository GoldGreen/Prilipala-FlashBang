...
public static class SharpExtensions
{
    ...
    public static float FixeTo(this float value, int digits)
    {
        return (float)Math.Round(value, digits);
    }

    public static bool IsOneOf<T>(this T value, params T[] keys)
    {
        return keys.Contains(value);
    }

    public static bool IsOneOf<T>(this T value, IEnumerable<T> keys)
    {
        return keys.Contains(value);
    }
    ...
    {
        float min = value / multiply;
        float max = value * multiply;
